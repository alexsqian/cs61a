...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 1:
        temp_variable1 = scheme_eval(expressions.first, env)
        if temp_variable1:
            return temp_variable1
        else:
            return False
    temp_variable2 = scheme_eval(expressions.first, env)
    if scheme_false(temp_variable2):
        return temp_variable2
    else:
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 0:
        return False
    temp_variable = scheme_eval(expressions.first, env)
    if scheme_true(temp_variable):
...
def make_let_frame(bindings, env):
    ...
    for i in bindings:
        check_form(i, 1, 2)
        if scheme_numberp(i[0]):
            raise SchemeError
        formals, values = i[0], i[1]
        formals_list.append(formals)
        values_list.append(scheme_eval(values, env))
    return env.make_child_frame(formals_list, values_list)
...
def check_formals(formals):
    ...
    checklist = []
    while index < len(formals):
        if not scheme_symbolp(formals[index]):
            raise SchemeError
        if formals[index] in checklist:
            raise SchemeError
        checklist.append(formals[index])
        index +=1
#################
# Dynamic Scope #
#################
