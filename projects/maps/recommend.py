...
def find_closest(location, centroids):
    ...
    """
    "*** YOUR CODE HERE ***"
    return min(centroids, key=lambda _: distance(_, location))

    

    #return min([distance(location, x) for x in centroids if True])
...
def group_by_centroid(restaurants, centroids):
    ...
    """
    "*** YOUR CODE HERE ***"
    #need a list like [[closest centroid, restaurant], [closest centroid, restaurant]]
    #to do that, call dictionary restaurant, get location of it, find nearest centroid
    list_of_restaurants = [[find_closest(restaurant_location(restaurant), centroids), restaurant] for restaurant in restaurants]

    return group_by_first(list_of_restaurants)

def find_centroid(restaurants):
    """Return the centroid of the locations of RESTAURANTS."""
    "*** YOUR CODE HERE ***"
    latitude = [restaurant_location(restaurant)[0] for restaurant in restaurants]
    longitude = [restaurant_location(restaurant)[1] for restaurant in restaurants]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(restaurant) for restaurant in new_cluster if restaurant !=0]
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for feature in feature_fns:
        predictor1[find_predictor(user, reviewed, feature)[0]] = [find_predictor(user, reviewed, feature)[1]]
    return max(predictor1, key=predictor1.get)
...
def rate_all(user, restaurants, feature_functions):
    ...
    "*** YOUR CODE HERE ***"
    def already_rated(restaurant):
            if restaurant in user_reviewed_restaurants(user, restaurants):
                    return user_rating(user, restaurant)
            else:
                return predictor(restaurants[restaurant])
    return {restaurant:already_rated(restaurant) for restaurant in restaurants}
