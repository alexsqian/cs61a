...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.container:
                self.ant = insect.ant
            elif type(insect) == QueenAnt:
                if insect.Impostor is True:
                    self.ant = None
                else:
                    return
            else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        "*** YOUR CODE HERE ***"
        ant = self.place.ant
        return self.place.ant is not None and ant.blocks_path is True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while throw_place != hive:
            if len(throw_place.bees) != 0 and self.min_range <= range_ant <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        "*** YOUR CODE HERE ***"
        bees_to_kill = list(self.place.bees)
...
# The WallAnt class
class WallAnt(Ant):
    """A Wall Ant that doesn't move and has high armor"""
    name = 'Wall'
    implemented = True
    food_cost = 4
    armor = 4
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        def damage_doubler(place):
            #print (place)
            if place.exit == None or place.entrance ==None:
                return
            elif not place.ant or place.ant in self.ants_doubled:
                return 
            else:
                place.ant.damage = 2*place.ant.damage
                self.ants_doubled.append(place.ant)
            if place.ant.container and type(place.ant.ant) != QueenAnt:
                place.ant.ant.damage = place.ant.ant.damage*2
                self.ants_doubled.append(place.ant)


        next_place = self.place
        while next_place.entrance:
            next_place = next_place.entrance
            damage_doubler(next_place)


        previous_place = self.place
        while previous_place.exit:
            previous_place = previous_place.exit
            damage_doubler(previous_place)

        if type(self.place.ant) == BodyguardAnt:
            damage_doubler(self.place)
