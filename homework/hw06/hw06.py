...
class Mobile:
    ...
    def weight(self):
        ...
        def weight_calculator(self):
            total_weight = self.left.contents.weight + self.right.contents.weight
            return total_weight
        return weight_calculator(self)

    
    def is_balanced(self):
        """True if and only if the mobile is balanced."""
        "*** YOUR CODE HERE ***"
        left_t = self.left.torque
        right_t = self.right.torque
        if (right_t == left_t) and self.left.contents.is_balanced() and self.right.contents.is_balanced():
            return True
        else:
            return False
